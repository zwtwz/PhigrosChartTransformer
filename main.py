...
enabled_levels = config.handlingLevels
...
def get_chart(metadata: dict, mode: int = 0, save_path: str = "") -> Union[None, int]:
    ...
    elif search_result_bpm == -2 and mode == 1:
        #多个谱面处理
        for chart in [i["chart"] for i in metadata["charts"] if i["chart"] == ""]:
            print("  处理 %s 难度" % chart["level"])
            possibleCharts = chart["possibleCharts"]

            #先检查是否有选项谱面已被选择
            charts_include_options = db.select("metadatas",
                                         where=lambda x: any([y["chart"] in possibleCharts for y in x["charts"]]),
                                         fields=["charts"])  # 返回 [{charts:[{chart: "chart1"}}...] 结构
            charts_include_options = [v["chart"] for v in [i["charts"] for i in charts_include_options]]  # 返回 ["chart1"...] 结构
            unselected_options = [i for i in possibleCharts if i not in charts_include_options]  # 筛选没用过的谱面
...
def info_handle(option=1):
    ...
    choice = ""
    if option != 0 or not use_auto_song_recognizing:
        #手动模式
    ...
    #根据模式生成musicList
    if option == 0:
        music_list = db.select("metadatas", where=lambda x: x["status"] == [-1,-1,-1])
        ignored_songs = [music["music"] for music in music_list]
    ...
    #option等于123对应的错误，与状态码指示的错误一一对应
        ...
        music_list = db.select("metadatas", where=lambda x: x["status"][option - 1] == 2)
    ...
    for i, music_file_name in enumerate(music_list):
        ...
        if option == 2 or option == 3:
            metadata = db.select("metadatas", where={"music": music_file_name})[0]
        else:
        ...
        if metadata["status"][2] == 0 or option == 3:
            result_of_chart_serach = get_chart(metadata, chart_search_mode)
    ...
    for new_music_file_name in new_music_list:
        ...
        for metadata in metadatas:
            for chart in metadata["charts"]:
                chart["chart"] = chartSearch.db.select(chart["level"], where={"old_filename": metadata["chart"]})[0]["chart"]
            metadata.update({
                "old_music_file_name": metadata["music"],
                "music": new_music_file_name,
                "illustration": illustrationSearch.db.select("illustrationLUT", where={"old_filename": metadata["illustration"]})[0]["filename"]
            })
        db.delete("metadatas", where={"music": metadata["music"]})
        db.append("metadatas", metadata)
    db.commit()
    ...
    while True:
        ...
        match choiceA:
            ...
            case "5":
                #错误处理
                def len_err(y):
                    return str(len(db.select("metadatas", where=lambda x: x["status"][y] == 2)))
                print("请选择要人工处理的错误")
                print("【1】谱面信息错误：%s个" % len_err(0))
                print("【2】曲绘未找到：%s个" % len_err(1))
                print("【3】谱面未找到：%s个" % len_err(2))
                print("【4】忽略现存的谱面信息错误")
                ...
                match choiceB:
                    ...
                    case "4":
                        ...
                        music_list = db.select("metadatas", where=lambda x: x["status"][0] == 2)
